...
class UdpServer(object):
	...
	#we have to modify this function for group security in order to create a con_req message with group information
    def trigger_conn_request(self, peer):
	    ...
	    if "group" in CONFIG:
		group=CONFIG["group"]
		group_len=str(len(group))
		do_send_msg(self.sock, "con_req", 1, peer["uid"], group_len+group+self.ipop_state["_fpr"]);
	    else:
		do_send_msg(self.sock, "con_req", 1, peer["uid"], self.ipop_state["_fpr"]);
